    ...
    for file_path in files:
        with open(file_path, "r") as f:
            try:
                ...
                cursor.execute(
                    """
                    INSERT OR IGNORE INTO advisories (id, summary, details, aliases, published)
                    VALUES (?, ?, ?, ?, ?)
                """,
                    (
                        advisory_id,
                ...
                for item in affected:
                    ...
                    # Join multiple fixed versions with a comma, or leave empty if none found
                    fixed_version_str = (
                        ", ".join(fixed_versions) if fixed_versions else ""
                    )

                    cursor.execute(
                        """
                        INSERT INTO affected_packages (advisory_id, ecosystem, name, fixed_version)
                        VALUES (?, ?, ?, ?)
                    """,
                        (
                            advisory_id,
